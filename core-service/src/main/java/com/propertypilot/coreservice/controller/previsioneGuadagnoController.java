...
import com.propertypilot.coreservice.dto.SendPrevisioneEmailRequest;
import com.propertypilot.coreservice.model.PrevisioneGuadagno;
import com.propertypilot.coreservice.service.PrevisioneEmailService;
...
public class PrevisioneGuadagnoController {
    ...
    @Autowired
    PrevisioneEmailService previsioneEmailService;
    ...
    @PostMapping("/sendPrevisioneEmail")
    public ResponseEntity<ResponseHandler<?>> sendEmail(@Valid @RequestBody SendPrevisioneEmailRequest req) {

        log.info("Richiesta invio email previsione: previsioneId={}, ownerEmail={}",
                req.previsioneId(), req.ownerEmail());

        previsioneEmailService.sendPrevisioneToOwner(req.previsioneId(), req.ownerEmail(), req.ownerName());

        return ResponseEntity.ok(ResponseHandler.success("Email inviata"));
    }
}